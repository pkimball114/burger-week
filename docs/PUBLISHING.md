...
Important: `data/burgers.csv` is still placeholder data. The app pads the Burger Week board to 124 burgers, so a full-looking deployment does not mean the data is official or complete.
...
For app, CSV, JSON, and photo updates:
...
1. Add or update rows in `data/burgers.csv`.
2. Put restaurant photos in `data/photos/`.
3. Reference each photo from the CSV using a relative path like:
...
```js
const cacheName = "burger-week-v9";
```

The cache bump matters because the app currently registers a service worker that precaches `data/burgers.csv`. Without a new cache name, returning visitors may keep seeing an older CSV even after GitHub Pages has deployed the newest files.
