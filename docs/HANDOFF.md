...
- The Map view uses Leaflet with all 124 CSV coordinates. Open now, Hide visited, search, and area filters update marker visibility, map-local detail checkboxes customize popup content, and a floating top-right map icon opens a full-screen map mode where the details controls are hidden; see `docs/MAP.md`.
